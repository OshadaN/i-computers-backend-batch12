...
export async function deleteProduct(req, res) {
    try {
        ...
        if (isAdmin(req)) {
            ...
            res.status(403).json({ message: "You need to login as an admin to delete products." });
            return;
        }
    } catch (error) {
...
export async function updateProduct(req, res) {
    try {
        ...
        if (isAdmin(req)) {
            ...
            res.status(403).json({ message: "You need to login as an admin to update products." });
            return;
        }
